...
async function packageWindows() {
  ...
  for (const arch of archs) {
    ...
    const nodeArch = getNodeArch(arch)

    try {
      run(`pnpm prepare:check -- --sidecar-host ${targetTriple} --arch ${nodeArch}`)
    } catch (e) {
      consola.error(
        colorize`Prepare sidecars failed for {cyan ${arch}}. Skipping packaging for this arch.`,
      )
      continue
    }

...
import { printClashXiaoy } from './utils'
...
const tasks: {
  ...
  { name: 'clash', func: () => resolve.clash(), retry: 5, required: true },
  { name: 'mihomo', func: () => resolve.clashMeta(), retry: 5, required: true },
  {
    name: 'mihomo-alpha',
    func: () => resolve.clashMetaAlpha(),
    retry: 5,
    required: false,
  },
  {
  ...
  { name: 'mmdb', func: () => resolve.mmdb(), retry: 5, required: true },
  { name: 'geoip', func: () => resolve.geoip(), retry: 5, required: true },
  { name: 'geosite', func: () => resolve.geosite(), retry: 5, required: true },
  {
    name: 'enableLoopback',
    ...
    try {
      ...
      if (i === task.retry - 1) {
        if (task.required !== false) {
          ...
          consola.error(`task::${task.name} skipped`, err)
        }
      } else {

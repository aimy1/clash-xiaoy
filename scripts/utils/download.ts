...
export const downloadFile = async (url: string, path: string) => {
  ...
  try {
    ...
    if (host.endsWith('github.com') || host.endsWith('api.github.com')) {
      urlsToTry.push(getGithubUrl(url))
    }
  } catch {
    // ignore
  }
  ...
  for (const u of urlsToTry) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        consola.debug(colorize`download {gray "${u}"} to {gray "${path}"}`)
        ...
        consola.success(
          colorize`download finished {gray "${u.split('/').at(-1)}"}`,
        )
        return
      } catch (error) {
        lastError = error
...
export const resolveSidecar = async (
  ...
  let { name, targetFile, tmpFile, exeFile, downloadURL } = await binInfo

  if (name === 'nyanpasu-service') {
    targetFile = targetFile.replace('nyanpasu-service', 'xiaoy-service')
  }
  ...
  if (!option?.force && (await fs.pathExists(sidecarPath))) {

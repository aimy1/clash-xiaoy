...
impl<F: Fn(DownloaderState)> Downloader<F> {
    ...
    async fn confirm_file_status(&self) -> Result<(String, u64), DownloaderError> {
            ...
            .and_then(|v| v.error_for_status())
        {
            ...
            Err(e) => {
                let mut inner = self.inner.write();
                inner.mode = DownloadMode::SingleThread;
                inner.total_size = 0;
                allow_multi_thread = false;
                tracing::warn!("Failed to HEAD {}: {}", self.url.as_str(), e);
                None
            }
        };
        ...
        if let Some(response) = response {
            ...
            if let Some(v) = headers.get(reqwest::header::CONTENT_LENGTH) {
        ...
        if allow_multi_thread {
            let range_probe = self
                .client
                .get(self.url.clone().as_str())
...

        {
            ...
            inner.mode = if allow_multi_thread && total_size > 0 {
    ...
    async fn download(&self) -> Result<(), DownloaderError> {
        ...
        if total_size == 0 && matches!(mode, DownloadMode::MultiThread) {
            let (_, size) = self.confirm_file_status().await?;
            total_size = size;
        }

        if total_size == 0 {
            let _ = self.confirm_file_status().await?;
        }
        ...
        let mut counts = if matches!(mode, DownloadMode::SingleThread) {
            1u64
        } else {
            let inner = self.inner.read();
            inner.semaphore.available_permits() as u64
        };
        let use_range = !matches!(mode, DownloadMode::SingleThread);

        if counts == 0 {
            return Err(DownloaderError::Other("No available permits".to_string()));
        }

        if use_range {
            counts = counts.min(total_size.max(1));
        }
...
impl SafeChunkThread for RwLock<ChunkThread> {
    ...
    async fn download_chunk(&self) -> Result<(), anyhow::Error> {
        ...
        let response = {
            let (client, url, start, end, use_range) = {
                let thread = self.read();

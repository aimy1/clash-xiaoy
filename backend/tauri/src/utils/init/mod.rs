...
pub fn init_config() -> Result<()> {
    ...
    crate::log_err!(dirs::clash_guard_overrides_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IClashTemp::template().0, Some("# clash-xiaoy"))?;
        }
        <Result<()>>::Ok(())
    }));

    crate::log_err!(dirs::clash_xiaoy_config_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IVerge::template(), Some("# clash-xiaoy"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::profiles_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &Profiles::default(), Some("# clash-xiaoy"))?;
        }
        <Result<()>>::Ok(())
    }));

...
pub fn resolve_setup(app: &mut App) {
    ...
    log::trace!("init widget manager");
    {
        ...
        tauri::async_runtime::spawn(async move {
            log_err!(crate::widget::setup(&app_handle, receiver).await);
        });
    }
...
pub fn create_main_window(app_handle: &AppHandle) {
    crate::utils::tray_debug_log::log("create_main_window called");
    crate::utils::tray_debug_log::log_window_lookup("create_main_window entry", app_handle, "main");
    log_err!(MainWindow.create(app_handle));
    crate::utils::tray_debug_log::log_window_lookup(
        "create_main_window after create",
        app_handle,
        "main",
    );
}
...
#[tracing_attributes::instrument(skip(app_handle))]
pub fn revive_main_window(app_handle: &AppHandle) {
    use tauri::PhysicalSize;

    crate::utils::tray_debug_log::log("Tray clicked -> reviveMainWindow");
    crate::utils::tray_debug_log::log_window_lookup("Revive entry", app_handle, "main");

    if app_handle.get_webview_window("main").is_none() {
        crate::utils::tray_debug_log::log("Revive: main window not found -> creating");
        log_err!(MainWindow.create(app_handle));
    }

    let Some(win) = app_handle.get_webview_window("main") else {
        crate::utils::tray_debug_log::log("Revive: main window still not found -> abort");
        return;
    };
    ...
    crate::utils::tray_debug_log::log("Revive: size reset");

    log_err!(win.center(), "revive center");

    log_err!(win.unminimize(), "revive unminimize");
    log_err!(win.show(), "revive show");
    crate::utils::tray_debug_log::log("Revive: show()");

    log_err!(win.set_focus(), "revive focus");
    crate::utils::tray_debug_log::log("Revive: focus()");

    crate::utils::tray_debug_log::log_webview_window_state("Revive exit", &win);
}

...
pub struct IVerge {
    ...
    /// background image (data url / url / file path)
...
impl IVerge {
    pub fn new() -> Self {
        match dirs::clash_xiaoy_config_path().and_then(|path| help::read_yaml::<IVerge, _>(&path)) {
            Ok(mut config) => {
    ...
    pub fn save_file(&self) -> Result<()> {
        help::save_yaml(
            &dirs::clash_xiaoy_config_path()?,
            &self,
            Some("# clash-xiaoy Config"),
        )
    }
}

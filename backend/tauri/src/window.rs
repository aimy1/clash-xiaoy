...
pub trait AppWindow {
    ...
    fn create(&self, app_handle: &AppHandle) -> Result<()> {
        tray_debug_log::log(format!(
            "AppWindow.create entry label={} url={}",
            self.label(),
            self.url()
        ));
        tray_debug_log::log_window_lookup("AppWindow.create entry", app_handle, self.label());
        if let Some(window) = app_handle.get_webview_window(self.label()) {
            ...
            tray_debug_log::log("AppWindow.create existing window: before unminimize/show/focus");
            tray_debug_log::log_webview_window_state(
                "AppWindow.create existing window (before)",
                &window,
            );
            ...
            tray_debug_log::log_webview_window_state(
                "AppWindow.create existing window (after)",
                &window,
            );
            return Ok(());
        }
        ...
        tray_debug_log::log("AppWindow.create creating new window");
        ...
        tray_debug_log::log(format!(
            "AppWindow.create before build label={} url={}",
            self.label(),
            self.url()
        ));
        ...
        match win_res {
            Ok(win) => {
                tray_debug_log::log("AppWindow.create after build ok");
                tray_debug_log::log_webview_window_state("AppWindow.create after build ok", &win);
...
>>>>>>> cca0f91e5b3534817b94d9736a4ac8fadf96b5b1
                ...
                if center.unwrap_or(true) {
                    tray_debug_log::log("AppWindow.create before center");
                    trace_err!(win.center(), "set win center");
                    tray_debug_log::log_webview_window_state("AppWindow.create after center", &win);
                }
                ...
                tray_debug_log::log("AppWindow.create exit ok");
            }
            Err(err) => {
                log::error!(target: "app", "failed to create window, {err:?}");
                tray_debug_log::log(format!("AppWindow.create build error: {err:?}"));
                if let Some(win) = app_handle.get_webview_window(self.label()) {
                    ...
                    tray_debug_log::log("AppWindow.create before destroy cleanup window");
                    tray_debug_log::log_webview_window_state(
                        "AppWindow.create cleanup (before destroy)",
                        &win,
                    );
                    log_err!(
                        win.destroy(),
                    ...
                    tray_debug_log::log("AppWindow.create after destroy cleanup window");
                }
            }
        }

...
impl UpdaterBuilder {
    ...
    pub async fn build(self) -> anyhow::Result<Updater> {
        ...
        let work_dir = crate::utils::dirs::app_data_dir()?
            .join("updates")
        ...
        let file = tokio::fs::File::create(work_dir.join(&artifact)).await?;
...
impl Updater {
    ...
    async fn decompress_and_set_permission(&self) -> anyhow::Result<()> {
        ...
        let path = self.work_dir.join(&self.artifact);
        ...
        .await??;
        let tmp_core = self.work_dir.join(format!(
            "{}{}",
            self.core_type,
    ...
    async fn replace_core(&self) -> anyhow::Result<()> {
        ...
        let target_core = crate::utils::dirs::app_data_dir()?.join(target_core);
        tracing::debug!("copying core to {:?}", target_core);
        let tmp_core_path = self.work_dir.join(format!(
            "{}{}",
            self.core_type,
            std::env::consts::EXE_SUFFIX
        ));

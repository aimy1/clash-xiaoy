...
pub fn run() -> std::io::Result<()> {
    ...
    tauri_plugin_deep_link::prepare("com.clash-xiaoy.app.dev");

    #[cfg(not(feature = "verge-dev"))]
    tauri_plugin_deep_link::prepare("com.clash-xiaoy.app");
    ...
    match &single_instance_result {
        Ok(Some(_)) => {
            ...
            if let Ok(data_dir) = crate::utils::dirs::app_data_dir() {
                ...
                let _ = std::fs::write(data_dir.join("port"), server::SERVER_PORT.to_string());
            }
        }
        Ok(None) => {
                    ...
                    if let Ok(port) = port_str.trim().parse::<u16>() {
                        let _ = tokio::runtime::Builder::new_current_thread()
                            ...
                            .block_on(async {
                                ...
                                for _ in 0..10 {
                                    if client
                                        .get(format!("http://127.0.0.1:{port}/show"))
                                        .send()
                                        .await
                                        .is_ok()
                                    {

...
abstract class BaseActivity<D : Design<*>> : AppCompatActivity(),
    ...
    override fun attachBaseContext(newBase: Context) {
        val store = UiStore(newBase)
        val configuration = Configuration(newBase.resources.configuration)

        when (store.language) {
            AppLanguage.Auto -> Unit
            AppLanguage.English -> configuration.setLocale(Locale.ENGLISH)
            AppLanguage.Chinese -> configuration.setLocale(Locale.SIMPLIFIED_CHINESE)
            AppLanguage.ChineseTraditional -> configuration.setLocale(Locale.TRADITIONAL_CHINESE)
            AppLanguage.Japanese -> configuration.setLocale(Locale.JAPANESE)
            AppLanguage.Korean -> configuration.setLocale(Locale.KOREAN)
            AppLanguage.Russian -> configuration.setLocale(Locale("ru"))
        }

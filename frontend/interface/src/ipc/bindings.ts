...
let mockVergeConfig: any = {}
const TAURI_INVOKE = async <T>(cmd: string, args?: any): Promise<T> => {
  try {
    return await REAL_TAURI_INVOKE<T>(cmd, args)
  } catch (e) {
    console.warn(`[Mock Bindings] Invoke '${cmd}' failed/mocked:`, e)
    // Return appropriate default values for common commands
    if (cmd === 'get_verge_config') return mockVergeConfig as T
    if (cmd === 'patch_verge_config') {
      const payload = args?.payload ?? {}
      mockVergeConfig = { ...mockVergeConfig, ...payload }
      return null as T
    }
    if (cmd === 'get_clash_info')
      return { port: 7890, server: '127.0.0.1', secret: '' } as unknown as T
...
const TAURI_API_EVENT = {
  ...REAL_TAURI_API_EVENT,
  listen: async <T>(event: string, handler: any) => {
    try {
      return await REAL_TAURI_API_EVENT.listen<T>(event, handler)
    } catch (e) {
      console.warn(`[Mock Bindings] Listen '${event}' failed/mocked`, e)
      return () => {} // Return no-op unlisten function
    }
  },
  once: async <T>(event: string, handler: any) => {
    try {
      return await REAL_TAURI_API_EVENT.once<T>(event, handler)
    } catch (e) {
      return () => {}
    }
  },
  emit: async (event: string, payload?: any) => {
    try {
      return await REAL_TAURI_API_EVENT.emit(event, payload)
    } catch (e) {
      // ignore
    }
  }
...
type __EventObj__<T> = {
  listen: (
    cb: REAL_TAURI_API_EVENT.EventCallback<T>,
  ) => ReturnType<typeof REAL_TAURI_API_EVENT.listen<T>>
  once: (
    cb: REAL_TAURI_API_EVENT.EventCallback<T>,
  ) => ReturnType<typeof REAL_TAURI_API_EVENT.once<T>>
  emit: null extends T
    ? (payload?: T) => ReturnType<typeof REAL_TAURI_API_EVENT.emit>
    : (payload: T) => ReturnType<typeof REAL_TAURI_API_EVENT.emit>
}

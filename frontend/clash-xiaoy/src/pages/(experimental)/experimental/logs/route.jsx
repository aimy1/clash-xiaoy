...
const InnerComponent = ({ logs, }) => {
    ...
    return (<div className="px-4 py-4">
      <div className="relative flex flex-col" data-slot="logs-virtual-list" style={{
        ...
        {virtualItems.map((virtualItem) => {
            ...
            return (<div key={virtualItem.key} ref={rowVirtualizer.measureElement} className={cn('absolute top-0 left-0 w-full select-text', 'rounded-xl border border-outline/30 bg-surface/50', 'px-3 py-2', 'transition-colors hover:bg-surface-variant/40')} data-index={virtualItem.index} data-slot="logs-virtual-item" style={{
                    transform: `translateY(${virtualItem.start}px)`,
                }}>
              <div className="flex items-center gap-2">
                <span className="min-w-20 text-xs text-zinc-500">{log.time || '--'}</span>
                <LogLevelBadge className="text-xs">{log.type}</LogLevelBadge>
              </div>

              <div className="mt-1 font-mono text-sm break-all">{log.payload}</div>
            </div>);
        })}
      </div>
    </div>);
    ...
    return (<div className="flex h-full flex-col">
      <div className="border-outline/40 bg-surface/70 flex items-center gap-3 border-b px-4 py-3">
        ...
        <div className="w-[280px] max-w-[50vw]">
          <Input variant="outlined" label="过滤（payload / type）" value={filterText} onChange={(e) => setFilterText(e.target.value)}/>
        </div>

        <Button variant="flat" onClick={() => (status ? disable() : enable())}>
          {status ? '暂停' : '继续'}
        </Button>

        <Button variant="flat" onClick={() => clean.mutate()} disabled={logs.length === 0 || clean.isPending}>

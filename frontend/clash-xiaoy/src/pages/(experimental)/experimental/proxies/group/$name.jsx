...
export const Route = createFileRoute('/(experimental)/experimental/proxies/group/$name')({
    component: RouteComponent,
});
function RouteComponent() {
    const { name: proxyGroupName } = Route.useParams();
    const { data: proxies } = useClashProxies();
    const currentGroup = proxies?.groups.find((group) => group.name === proxyGroupName);
    const { viewportRef } = useScrollArea();
    // define the number of lanes based on the container breakpoint
    const lanes = useContainerBreakpointValue(viewportRef, {
        xs: 2,
        sm: 3,
        md: 4,
        lg: 5,
    ...
    const handleScrollToCurrentNode = useCallback(() => {
        const index = currentGroup?.all?.findIndex((proxy) => proxy.name === currentGroup?.now);
    ...
    return (<>
      ...
      <div className="relative m-2" data-slot="proxies-virtual-list" style={{
            width: 'calc(100% - 16px)',
        ...
        {virtualItems.map((virtualItem) => {
            ...
            return (<div key={virtualItem.index} ref={virtualizer.measureElement} className="group absolute top-0 left-0 p-1" style={{
                    transform: `translateY(${virtualItem.start}px)`,
                    width: `${100 / lanes}%`,
                    left: `${virtualItem.lane * (100 / lanes)}%`,
                }} data-index={virtualItem.index} data-slot="proxies-virtual-item" data-active={String(proxy.name === currentGroup?.now)}>

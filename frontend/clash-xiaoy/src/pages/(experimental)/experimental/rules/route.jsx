...
const InnerComponent = ({ rules, isLoading, isError, errorText, }) => {
    ...
    if (rules.length === 0) {
        ...
        <div className="bg-surface/40 border-outline/40 flex flex-col items-center gap-2 rounded-xl border px-3 py-10 text-center">
          ...
          <div className="text-xs text-zinc-500">请确认 Clash Core 正在运行，且外部控制接口可用</div>
        </div>
      </div>);
    }
    ...
    return (<div className="px-4 py-4">
      <div className="relative flex flex-col" data-slot="rules-virtual-list" style={{
        ...
        {virtualItems.map((virtualItem) => {
            ...
            return (<div key={virtualItem.key} ref={rowVirtualizer.measureElement} className={cn('absolute top-0 left-0 w-full select-text', 'rounded-xl border border-outline/30 bg-surface/50', 'px-3 py-2', 'transition-colors hover:bg-surface-variant/40')} data-index={virtualItem.index} data-slot="rules-virtual-item" style={{
                    transform: `translateY(${virtualItem.start}px)`,
                }}>
              <div className="flex items-start gap-3">
                <div className="min-w-12 text-xs text-zinc-500 pt-1">{virtualItem.index + 1}</div>
                <div className="min-w-0 flex-1">
                  <div className="font-mono break-all text-sm text-primary">{rule.payload || '-'}</div>
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <span className="rounded-md bg-surface/70 px-2 py-0.5 text-xs text-zinc-600">{rule.type || '-'}</span>
                    <span className="font-mono text-xs text-zinc-500 break-all">{rule.proxy || '-'}</span>
                  </div>
                </div>
              </div>
            </div>);
...
function RouteComponent() {
    ...
    const rules = useMemo(() => {
        ...
        return rawRules.filter((rule) => {
            ...
            return payload.includes(query) || type.includes(query) || proxy.includes(query);
        });
    ...
    return (<div className="flex h-full flex-col">
      <div className="border-outline/40 bg-surface/70 flex items-center gap-3 border-b px-4 py-3">
        ...
        <div className="w-[280px] max-w-[50vw]">
          <Input variant="outlined" label="过滤（payload / type / proxy）" value={filterText} onChange={(e) => setFilterText(e.target.value)}/>
        </div>
      ...
      <AppContentScrollArea className="flex-1">
        <InnerComponent rules={rules} isLoading={isLoading} isError={isError} errorText={errorText}/>

...
const ProxyProviderCard = ({
  ...
  return (
    <div className="rounded-xl border border-outline/30 bg-surface/60 p-4 shadow-sm">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {provider.testUrl && (
          <span className="truncate rounded-md bg-surface/70 px-2 py-0.5 text-xs text-zinc-600">
...
function RouteComponent() {
  ...
  const updateOne = async (key: string, fn: () => Promise<void>) => {
    setActionError(null)
  ...
  const updateAll = async (
    kind: 'proxies' | 'rules',
    items: [string, { mutate: () => Promise<void> }][],
  ) => {
    setActionError(null)
  ...
  const errorText = (() => {
    if (proxiesProvider.isError) return String(proxiesProvider.error)
    if (rulesProvider.isError) return String(rulesProvider.error)
    return null
  })()
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-outline/40 bg-surface/70 flex items-center gap-3 border-b px-4 py-3">
        ...
        <Button
          variant="flat"
          onClick={() => Promise.all([proxiesProvider.refetch(), rulesProvider.refetch()])}
          disabled={proxiesProvider.isFetching || rulesProvider.isFetching}
        >
        ...
        <div className="flex flex-col gap-5 p-5 sm:p-6 lg:p-8">
          <Card className="bg-surface/70 border-outline/40 shadow-sm">
            <CardContent className="flex flex-col gap-2 p-5">
              ...
              {actionError && (
                <div className="rounded-lg border border-yellow-500/30 bg-yellow-500/10 px-3 py-2 text-sm text-yellow-700">
                  操作失败：{actionError}
                </div>
              )}
            </CardContent>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="flat"
                onClick={() => updateAll('proxies', proxiesItems)}
                disabled={updatingAll.proxies || proxiesItems.length === 0}
              >
          ...
          {proxiesItems.length === 0 && !proxiesProvider.isLoading ? (
            <div className="bg-surface/40 border-outline/40 rounded-xl border px-3 py-10 text-center">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              {proxiesItems.map(([key, provider]) => (
                <ProxyProviderCard
                  key={key}
                  provider={provider}
                  updating={Boolean(updating[`proxies:${key}`])}
                  onUpdate={() => updateOne(`proxies:${key}`, provider.mutate)}
                />
              ))}
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-3 pt-2">
            ...
            <div className="flex items-center gap-2">
              <Button
                variant="flat"
                onClick={() => updateAll('rules', rulesItems)}
                disabled={updatingAll.rules || rulesItems.length === 0}
              >
          ...
          {rulesItems.length === 0 && !rulesProvider.isLoading ? (
            <div className="bg-surface/40 border-outline/40 rounded-xl border px-3 py-10 text-center">
            ...
            <div className={cn('grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3')}>
              {rulesItems.map(([key, provider]) => (
                <RulesProviderCard
                  key={key}
                  provider={provider}
                  updating={Boolean(updating[`rules:${key}`])}
                  onUpdate={() => updateOne(`rules:${key}`, provider.mutate)}
                />
              ))}
            </div>
          )}

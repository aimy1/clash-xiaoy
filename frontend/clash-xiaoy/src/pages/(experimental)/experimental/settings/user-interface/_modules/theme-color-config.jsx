...
export default function ThemeColorConfig() {
    ...
    return (<SettingsCard data-slot="theme-color-config-card">
      <SettingsCardContent className="flex items-center justify-between px-3" data-slot="theme-color-config-card-content">
        ...
        <DropdownMenu open={open} onOpenChange={setOpen}>
          <DropdownMenuTrigger asChild>
            <Button className="flex items-center gap-2 px-4" variant="flat">
              <span className="size-4 rounded" style={{ backgroundColor: themeColor }}/>
          ...
          <DropdownMenuContent className="flex flex-col gap-4 rounded-2xl p-4">
            <Wheel data-slot="theme-color-config-colorful" color={cachedThemeColor} onChange={(color) => {
            setCachedThemeColor(color.hex);
        }}/>

            <Button className="flex items-center justify-center gap-2" variant="flat" onClick={handleSubmit}>

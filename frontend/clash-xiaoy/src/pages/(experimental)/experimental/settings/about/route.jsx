...
import { collectEnvs, openThat, useClashInfo, useClashVersion, } from '@nyanpasu/interface';
import { SettingsTitle, SettingsTitlePlaceholder, } from '../_modules/settings-title';
export const Route = createFileRoute('/(experimental)/experimental/settings/about')({
...
const InfoRow = ({ label, value, copyValue, }) => {
    return (<div className="flex items-start justify-between gap-3">
      ...
      <div className="min-w-0 flex items-center gap-2">
        ...
        {copyValue && (<Button variant="basic" className="h-8 min-w-0 px-3 text-xs" onClick={() => safeCopy(copyValue)}>
            复制
          </Button>)}
      </div>
    </div>);
...
function RouteComponent() {
    ...
    const appName = packageJson?.name ?? 'clash-xiaoy';
    ...
    const clashVariantText = clashVersion.data
        ? [clashVersion.data.meta ? 'meta' : null, clashVersion.data.premium ? 'premium' : null]
            .filter(Boolean)
            .join(' / ') || 'standard'
        : '--';
    return (<>
      ...
      <div className="flex flex-col gap-5 px-3 pb-8 sm:px-5">
        <Card className="bg-surface/70 border-outline/40 shadow-sm">
          <CardContent className="flex flex-col gap-4 p-5">
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-outline/30 bg-surface/50 p-4">
              ...
              <InfoRow label="Nightly" value={String(window.__IS_NIGHTLY__ ?? false)} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-surface/60 border-outline/30 shadow-sm">
          <CardContent className="flex flex-col gap-4 p-5">
            ...
            {clashInfo.isError || clashVersion.isError ? (<div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-600">
                {clashInfo.isError && <div>Clash Info: {String(clashInfo.error)}</div>}
                {clashVersion.isError && (<div>Clash Version: {String(clashVersion.error)}</div>)}
              </div>) : (<div className="flex flex-col gap-3 rounded-xl border border-outline/30 bg-surface/50 p-4">
        ...
        <Card className="bg-surface/60 border-outline/30 shadow-sm">
          <CardContent className="flex flex-col gap-4 p-5">
            ...
            {envsError ? (<div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-sm text-red-600">
                {envsError}
              </div>) : envs ? (<div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div className="rounded-xl border border-outline/30 bg-surface/50 p-4">
                  ...
                  <div className="mt-3 flex flex-col gap-2">
                    {coreEntries.length === 0 ? (<div className="text-sm text-zinc-500">--</div>) : (coreEntries.map(([k, v]) => (<InfoRow key={k} label={k} value={v} copyValue={v} />)))}
                  </div>
                </div>
              </div>) : (<div className="rounded-xl border border-outline/30 bg-surface/50 px-4 py-10 text-center text-sm text-zinc-500">
                加载中...
              </div>)}
          </CardContent>
        </Card>
        ...
        <Card className="bg-surface/60 border-outline/30 shadow-sm">
          <CardContent className="flex flex-col gap-4 p-5">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button variant="basic" onClick={() => envs && safeCopy(JSON.stringify(envs.build_info ?? {}, null, 2))} disabled={!envs}>
                复制全部
              </Button>
            </div>

            {envs ? (<div className={cn('rounded-xl border border-outline/30 bg-surface/50 p-4', 'flex flex-col gap-2')}>
                {buildInfoEntries.length === 0 ? (<div className="text-sm text-zinc-500">--</div>) : (buildInfoEntries.map(([k, v]) => (<InfoRow key={k} label={k} value={v} copyValue={v} />)))}
              </div>) : (<div className="rounded-xl border border-outline/30 bg-surface/50 px-4 py-10 text-center text-sm text-zinc-500">
                加载中...
              </div>)}
          </CardContent>

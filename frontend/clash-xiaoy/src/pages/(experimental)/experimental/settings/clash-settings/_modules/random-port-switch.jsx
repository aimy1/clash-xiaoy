...
export default function RandomPortSwitch() {
    ...
    const handleRandomPort = async () => {
        try {
            ...
            message(enableRandomPort.value
    ...
    return (<SettingsCard data-slot="random-port-switch-card">
      <SettingsCardContent className="flex items-center justify-between px-3" data-slot="random-port-switch-card-content">
        <div>{m.settings_clash_settings_random_port_label()}</div>

        <Switch checked={Boolean(enableRandomPort.value)} onCheckedChange={handleRandomPort} loading={enableRandomPort.isPending}/>
      </SettingsCardContent>
    </SettingsCard>);
}

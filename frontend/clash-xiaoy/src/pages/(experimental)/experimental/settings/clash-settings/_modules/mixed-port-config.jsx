...
import { SettingsCard, SettingsCardAnimatedItem, SettingsCardContent, } from '../../_modules/settings-card';
...
export default function MixedPortConfig() {
    ...
    const currentMixedPort = useMemo(() => {
        return (clashConfig.query.data?.['mixed-port'] ||
            mixedPort.value ||
            DEFAULT_MIXED_PORT);
    ...
    return (<SettingsCard data-slot="mixed-port-config-card">
      <SettingsCardContent className="px-2" data-slot="mixed-port-config-card-content">
        <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
          <Controller name="mixedPort" control={form.control} render={({ field, fieldState }) => {
            const handleChange = (value) => {
                field.onChange(value);
            };
            return (<>
                  <NumericInput variant="outlined" label={m.settings_clash_settings_mixed_port_label()} value={field.value} onChange={handleChange} allowNegative={false} decimalScale={0}/>

                  <AnimatePresence>
                    {fieldState.error && (<SettingsCardAnimatedItem className="text-error">
          ...
          <AnimatePresence initial={false}>
            {form.formState.isDirty && (<SettingsCardAnimatedItem>
                <div className="flex justify-end gap-2 pt-1">
                  <Button type="button" onClick={handleReset}>
                    {m.common_reset()}
                  </Button>

                  <Button variant="raised" onClick={handleSubmit} loading={form.formState.isSubmitting}>
                    {m.common_apply()}
                  </Button>
                </div>
              </SettingsCardAnimatedItem>)}

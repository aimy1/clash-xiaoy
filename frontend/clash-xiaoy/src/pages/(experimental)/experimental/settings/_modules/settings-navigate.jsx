...
const StyleClashMeta = () => {
    return (<img src={ClashMeta} alt="Clash Meta" className="size-8 grayscale-50"/>);
};
const StyleLogo = () => {
    return (<LogoSvg className="[&_#element]:fill-primary [&_#bg]:fill-surface size-8"/>);
};
const ROUTES = [
    {
        label: 'System Proxy',
    ...
    {
        label: 'About',
        description: 'About the clash-xiaoy',
...
export default function SettingsNavigate() {
    ...
    return (<div className="flex flex-col gap-2 p-2">
      {ROUTES.map((route) => (<Button key={route.href} variant="fab" data-active={String(location.pathname === route.href)} asChild>
          <Link className={cn('h-16', 'flex items-center gap-2', 'data-[active=true]:bg-surface-variant/80', 'data-[active=false]:bg-transparent', 'data-[active=false]:shadow-none', 'data-[active=false]:hover:shadow-none', 'data-[active=false]:hover:bg-surface-variant/30')} to={route.href}>

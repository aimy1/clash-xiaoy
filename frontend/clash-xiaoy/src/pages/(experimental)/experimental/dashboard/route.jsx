...
import { SystemProxyButton, TunModeButton } from '@/components/settings/system-proxy';
import parseTraffic from '@/utils/parse-traffic';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { AppContentScrollArea } from '@/components/ui/scroll-area';
import { useVisibility } from '@/hooks/use-visibility';
import { MAX_TRAFFIC_HISTORY, openThat, useClashConnections, useClashProxies, useClashTraffic, useClashWSContext, useIPSB, useProxyMode, useSetting, } from '@nyanpasu/interface';
...
function RouteComponent() {
    ...
    const { data: ipData, mutate: refreshIP, isValidating: ipRefreshing } = useIPSB();
    ...
    const padData = useMemo(() => (data = [], max) => Array(Math.max(0, max - data.length))
        .fill(0)
        .concat(data.slice(-max)), []);
    const downData = useMemo(() => padData(clashTraffic?.map((item) => item.down), MAX_TRAFFIC_HISTORY), [clashTraffic, padData]);
    const upData = useMemo(() => padData(clashTraffic?.map((item) => item.up), MAX_TRAFFIC_HISTORY), [clashTraffic, padData]);
    const activeProxyMode = useMemo(() => {
        const current = Object.entries(proxyMode).find(([, enabled]) => enabled)?.[0];
        return current ?? 'rule';
    }, [proxyMode]);
    ...
    const currentDelay = useMemo(() => {
        if (!currentNode?.history?.length)
            return 0;
        return currentNode.history[currentNode.history.length - 1].delay ?? 0;
    }, [currentNode]);
    const currentNodeLabel = proxyMode.direct ? 'DIRECT' : currentNode?.name ?? '未选择';
    ...
    return (<AppContentScrollArea>
      <div className="h-dvh p-4 sm:p-6">
        <div className="mx-auto w-full max-w-5xl">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <TrafficCard className="min-h-44 lg:col-span-2" downData={downData} upData={upData} downTotal={downTotal} upTotal={upTotal} visible={visible}/>

            <Card className={cn('cyber-glass cyber-box-glow')}>
              <CardContent className="gap-2">
                <div className="flex items-center justify-between">
                  <div className="text-sm font-semibold">当前节点</div>
                  <Button className="h-8 px-3" variant="basic" onClick={() => navigate({ to: '/experimental/proxies' })}>
                    切换
                  </Button>
                </div>
                <div className="truncate text-lg font-bold tabular-nums" title={currentNodeLabel}>
                  {currentNodeLabel}
                </div>
                <div className="text-on-surface-variant flex items-center justify-between text-sm">
                  <div>延迟</div>
                  <div className="tabular-nums">
                    {proxyMode.direct ? '-' : currentDelay > 0 ? `${currentDelay} ms` : 'Timeout'}
                  </div>
                </div>
                <div className="text-on-surface-variant flex items-center justify-between text-sm">
                  <div>节点类型</div>
                  <div className="tabular-nums uppercase">{currentNode?.type ?? '-'}</div>
                </div>
                <Button className="h-8" variant="flat" onClick={() => {
            ...
            <Card className={cn('cyber-glass cyber-box-glow')}>
              <CardContent className="gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-on-surface-variant text-xs uppercase">{activeProxyMode}</div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {['rule', 'global', 'direct'].map((mode) => (<Button key={mode} className="h-8 min-w-0 px-0 text-xs uppercase" variant={activeProxyMode === mode ? 'flat' : 'basic'} onClick={() => {
                updateProxyMode(mode);
            }}>
                      {mode}
                    </Button>))}
                </div>
                <div className="text-on-surface-variant text-sm">点击可直接切换代理模式</div>
              </CardContent>
            </Card>
            ...
            <Card className={cn('cyber-glass cyber-box-glow lg:col-span-3')}>
              <CardContent className="gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="mr-auto text-sm font-semibold">系统代理与 TUN</div>
                  <div className="rounded-full bg-surface px-3 py-1 text-xs">
            ...
            : '-'}
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <Button className="h-9" variant="flat" loading={releaseLoading} onClick={() => {
            refreshLatestRelease();
        }}>
                    检查最新版本
                  </Button>
                  <Button className="h-9" variant="basic" onClick={() => {
            openThat(latestRelease?.html_url ??
            ...
            <Card className={cn('cyber-glass cyber-box-glow')}>
              <CardContent className="gap-2">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="font-semibold tabular-nums">{parseTraffic(downTotal).join(' ')}</div>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <div className="text-on-surface-variant">上传总量</div>
                  <div className="font-semibold tabular-nums">{parseTraffic(upTotal).join(' ')}</div>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <div className="text-on-surface-variant">当前连接数</div>
                  <div className="font-semibold tabular-nums">{activeConnections}</div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className={cn('cyber-glass cyber-box-glow lg:col-span-2')}>
              <CardContent className="gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button className="h-8 px-3" variant="basic" loading={ipRefreshing} onClick={() => {
            refreshIP();
        }}>
                    刷新
                  </Button>
                </div>
                <div className="text-2xl font-bold tabular-nums">{ipData?.ip ?? '--.--.--.--'}</div>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                  <div className="rounded-xl bg-surface/70 p-2">
                    <div className="text-on-surface-variant text-xs">国家/地区</div>
                    <div className="truncate text-sm font-medium">{ipData?.country ?? '-'}</div>
                  </div>
                  <div className="rounded-xl bg-surface/70 p-2">
                    <div className="text-on-surface-variant text-xs">运营组织</div>
                    <div className="truncate text-sm font-medium">{ipData?.organization ?? '-'}</div>
                  </div>
                  <div className="rounded-xl bg-surface/70 p-2">

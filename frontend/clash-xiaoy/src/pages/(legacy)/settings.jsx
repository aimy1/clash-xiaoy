...
function SettingPage() {
    ...
    const GithubIcon = () => {
        const toGithubRepo = useLockFn(() => {
            return openThat('https://github.com/aimy1/clash-xiaoy');
        });
        return (<IconButton color="inherit" title="@aimy1/clash-xiaoy" onClick={toGithubRepo}>
    ...
    const FeedbackIcon = () => {
        const toFeedback = useLockFn(async () => {
            ...
            const formattedEnv = encodeURIComponent(formatEnvInfos(envs)
                ...
                .map((v) => `> ${v}`)
                .join('\n'));
            return openThat(`mailto:aisaniya@proton.me?subject=${encodeURIComponent('clash-xiaoy feedback')}&body=${formattedEnv}`);
        });
        return (<IconButton color="inherit" title={t('Feedback')} onClick={toFeedback}>
    ...
    const HotkeyButton = () => {
        ...
        <IconButton color="inherit" title={t('Hotkeys')} onClick={() => setOpen(true)}>
    ...
    const TrayIconButton = () => {
        ...
        <IconButton color="inherit" title={t('Tray Icons')} onClick={() => setOpen(true)}>
          <MdiTrayFull fontSize="inherit"/>
        </IconButton>
      </>);
    };
    return (<BasePage title={t('Settings')} header={<div className="flex gap-1">
          <TrayIconButton />
          <HotkeyButton />
          <FeedbackIcon />
          <GithubIcon />

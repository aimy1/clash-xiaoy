...
function ProvidersPage() {
    ...
    return (<BasePage title={t('Providers')}>
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <Chip className="!h-10 truncate !rounded-full !p-2 !text-lg font-bold" label={`${t(`Proxies Providers`)} (${Object.entries(proxiesProvider.data ?? {}).length})`}/>

          <UpdateProxiesProviders />
        </div>

        {proxiesProvider.data && (<Grid container spacing={2}>
            {Object.entries(proxiesProvider.data).map(([name, provider]) => (<Grid key={name} className="w-full" size={{
                    sm: 12,
                    md: 6,
                    lg: 4,
                    xl: 3,
        ...
        <div className="flex items-center justify-between">
          <Chip className="!h-10 truncate !rounded-full !p-2 !text-lg font-bold" label={`${t(`Rules Providers`)} (${Object.entries(rulesProvider.data ?? {}).length})`}/>

          <UpdateProviders />
        </div>

        {rulesProvider.data && (<Grid container spacing={2}>
            {Object.entries(rulesProvider.data).map(([name, provider]) => (<Grid key={name} size={{
                    sm: 12,
                    md: 6,
                    lg: 4,
                    xl: 3,

...
function SideBar() {
    ...
    return (<TextField hiddenLabel fullWidth autoComplete="off" spellCheck="false" placeholder={t('Filter conditions')} className="!pb-0" sx={{ input: { py: 1.2, fontSize: 14 } }} value={proxiesFilter || ''} onChange={(e) => setProxiesFilter(!e.target.value.trim().length ? null : e.target.value)} slotProps={{
            input: {
                sx: (theme) => ({
                    borderRadius: 7,
...
function ProxyPage() {
    ...
    const handleSwitch = useLockFn(async (key) => {
        await upsert(key);
    });
    const Header = useMemo(() => {
        return (<div className="flex items-center gap-1">
        <ToggleButtonGroup color="primary" size="small" exclusive onChange={(_, newValue) => handleSwitch(newValue)}>
          {Object.entries(proxyMode).map(([key, enabled], index) => (<ToggleButton key={key} className={cn('flex justify-center gap-0.5 !px-3', index === 0 && '!rounded-l-full', index === Object.entries(proxyMode).length - 1 &&
                    '!rounded-r-full')} value={key} selected={enabled}>
    ...
    return (<SidePage title={t('Proxy Groups')} leftViewportRef={leftViewportRef} rightViewportRef={rightViewportRef} header={Header} sideBar={<SideBar />} side={hasProxies &&
            proxyMode.rule && (<GroupList scrollRef={leftViewportRef}/>)} portalRightRoot={hasProxies &&
            !proxyMode.direct && (<div className={cn('absolute z-10 flex w-full items-center justify-between px-4 py-2 backdrop-blur', 'bg-gray-200/30 dark:bg-gray-900/30', '!rounded-t-2xl')}>
            <div className="flex items-center gap-4">
              {group?.name && <ProxyGroupName name={group?.name}/>}
            </div>

            <div className="flex gap-2">
              <ScrollCurrentNode onClick={() => {
      ...
      {!proxyMode.direct ? (hasProxies ? (<>
            <NodeList ref={nodeListRef} scrollRef={rightViewportRef}/>

            <DelayButton onClick={handleDelayClick}/>
          </>) : (<ContentDisplay className="absolute" message={t('No Proxies')}/>)) : (<ContentDisplay className="absolute" message={t('Direct Mode')}/>)}
    </SidePage>);
}

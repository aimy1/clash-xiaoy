...
import { z } from 'zod';
import { atomChainsSelected, atomGlobalChainCurrent, } from '@/components/profiles/modules/store';
import NewProfileButton from '@/components/profiles/new-profile-button';
import { AddProfileContext, } from '@/components/profiles/profile-dialog';
...
function ProfilePage() {
    ...
    const handleGlobalProfileUpdate = useLockFn(async () => {
        startGlobalUpdate(async () => {
            const remoteProfiles = profiles.clash?.filter((item) => item.type === 'remote') || [];
            const updates = [];
    ...
    return (<SidePage title={t('Profiles')} flexReverse header={<div className="flex items-center gap-2">
          <RuntimeConfigDiffDialog open={runtimeConfigViewerOpen} onClose={() => setRuntimeConfigViewerOpen(false)}/>
          <IconButton className="h-10 w-10" color="inherit" title={t('Runtime Config')} onClick={() => {
                setRuntimeConfigViewerOpen(true);
            }}>
            <MdiTextBoxCheckOutline />
          </IconButton>
          <Badge variant="dot">
            <Button size="small" variant={globalChain ? 'contained' : 'outlined'} onClick={handleGlobalChainClick} startIcon={<Public />}>
              {t('Global Proxy Chains')}
            </Button>
          </Badge>
        </div>} side={hasSide && <ProfileSide onClose={handleSideClose}/>}>
      <AnimatePresence initial={false} mode="sync">
        <GlobalUpdatePendingContext.Provider value={globalUpdatePending}>
          <div className="flex flex-col gap-4 p-6">
            <QuickImport />

            {profiles && (<Grid container spacing={2}>
                {profiles.clash?.map((item) => (<Grid key={item.uid} size={{
                    xs: 12,
                    sm: 12,
                    ...
                    <motion.div key={item.uid} layoutId={`profile-${item.uid}`} layout="position" initial={false}>
                      <ProfileItem item={item} onClickChains={onClickChains} selected={query.data?.current?.includes(item.uid)} 
      ...
      <AddProfileContext.Provider value={addProfileCtxValue}>
        <div className="!fixed right-8 bottom-8 flex flex-col gap-2 items-center">
          <FloatingButton className="!static flex size-11 !min-w-fit" sx={((theme) => ({
          ...
          <NewProfileButton className="!static size-11 !min-w-fit"/>
        </div>

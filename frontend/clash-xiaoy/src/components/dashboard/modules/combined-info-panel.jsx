...
import { Box, Button, CircularProgress, IconButton, Paper, Tooltip, } from '@mui/material';
...
function LatencyTag({ name, value }) {
...
export const CombinedInfoPanel = ({ timingData, refreshCount }) => {
    const { t } = useTranslation();
    ...
    return (<Paper className="relative flex !h-full gap-4 !rounded-3xl p-4 select-text cyber-glass overflow-hidden">
      ...
      <div className="w-40 flex flex-col justify-center gap-1.5">
        {Object.entries(timingData).map(([name, value]) => (<LatencyTag key={name} name={name} value={value}/>))}
      </div>
    </Paper>);
};

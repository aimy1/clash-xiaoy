...
export const TimingPanel = ({ data }) => {
    return (<Paper className="!h-full !rounded-3xl p-4 cyber-glass">
      <div className="flex h-full flex-col gap-2">
        {Object.entries(data).map(([name, value]) => (<LatencyTag key={name} name={name} value={value}/>))}
      </div>
    </Paper>);
};

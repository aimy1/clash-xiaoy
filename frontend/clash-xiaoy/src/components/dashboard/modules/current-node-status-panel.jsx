...
export const CurrentNodeStatusPanel = () => {
    ...
    return (<Paper className="flex !h-full flex-col justify-between gap-2 !rounded-3xl p-3 cyber-glass cursor-pointer ios-motion ios-pressable" onClick={() => navigate({ to: '/proxies' })}>
      <div className="flex items-center justify-between px-1">
        ...
        {currentNode && (<Chip label={currentNode.type} size="small" variant="outlined" sx={{
                height: 20,
                fontSize: '0.7rem',
      ...
      {currentNode ? (<div className="flex flex-col gap-1 px-1">
          <div className="truncate text-lg font-bold text-[var(--text-primary)]" title={currentNode.name}>
            {currentNode.name}
          </div>
          <div className="flex items-center gap-2 text-sm text-[var(--text-sub)]">
            <Speed sx={{ fontSize: 16, opacity: 0.7 }}/>
            {delay > 0 ? (<Box component="span" sx={{ ...delayColor, fontWeight: 600 }}>
                {delay} ms
              </Box>) : (<Box component="span" sx={{ color: 'error.main', fontWeight: 600 }}>
                Timeout
              </Box>)}
          </div>
        </div>) : data ? (<div className="flex flex-1 items-center justify-center text-[var(--text-sub)] text-sm">
          {t('No Node Selected')}

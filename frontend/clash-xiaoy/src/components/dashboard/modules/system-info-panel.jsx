...
import { collectEnvs } from '@nyanpasu/interface';
...
export const SystemInfoPanel = () => {
    ...
    return (<Paper className="flex !h-full flex-col justify-between gap-2 !rounded-3xl p-3 cyber-glass">
      ...
      {envs ? (<div className="flex flex-col gap-2">
          ...
          {envs.device.gpu && envs.device.gpu.length > 0 && (<InfoItem label="GPU" value={envs.device.gpu[0]}/>)}
          <InfoItem label="Memory" value={envs.device.memory}/>
          {typeof envs.device.uptime === 'number' && (<InfoItem label="Uptime" value={formatUptime(envs.device.uptime)}/>)}

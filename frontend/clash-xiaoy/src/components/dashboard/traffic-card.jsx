...
export const TrafficCard = ({ upData, downData, upTotal, downTotal, className, visible = true, }) => {
    const { t } = useTranslation();
    return (<Paper className={cn('relative overflow-hidden !rounded-3xl p-4 cyber-glass', className)}>
      <div className="absolute inset-0 opacity-40">
        <Sparkline data={downData} className="absolute inset-0 h-full w-full" color="var(--success)" visible={visible}/>
        <Sparkline data={upData} className="absolute inset-0 h-full w-full" color="var(--warning)" visible={visible}/>
      </div>

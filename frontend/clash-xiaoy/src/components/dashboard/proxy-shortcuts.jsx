...
const TitleComp = () => {
    ...
    const { query: { data: clashConfigs }, } = useClashConfig();
    ...
    return (<div className="flex items-center justify-between gap-2 px-1">
      <div className="text-sm font-semibold text-[var(--text-title)]">
        {t('Proxy Takeover Status')}
      </div>

      <Chip color={status.color} size="small" className="!h-5" sx={{
            span: {
                padding: '0 8px',
            },
        }} label={status.label}/>
...
export const ProxyShortcuts = () => {
    ...
    return (<Paper className="flex !h-full flex-col justify-between gap-2 !rounded-3xl p-3 cyber-glass">
      <TitleComp />

      <div className="grid grid-cols-2 gap-3">
        <PaperSwitchButton checked={systemProxy.value || false} onClick={handleSystemProxy} sxPaper={{
            borderRadius: 16,
          ...
          <div className="relative flex w-full flex-col gap-1.5">
            ...
            <span className={`absolute right-0 top-0 inline-flex h-5 min-w-10 items-center justify-center gap-1 rounded-full border px-2 text-[10px] font-mono tracking-wider select-none shadow-sm transition-colors ${systemProxy.value
        ...
        <PaperSwitchButton checked={tunMode.value || false} onClick={handleTunMode} sxPaper={{
            borderRadius: 16,
            border: '1px solid var(--border)',
          ...
          <div className="relative flex w-full flex-col gap-1.5">
            ...
            <span className={`absolute right-0 top-0 inline-flex h-5 min-w-10 items-center justify-center gap-1 rounded-full border px-2 text-[10px] font-mono tracking-wider select-none shadow-sm transition-colors ${tunMode.value
            ? 'bg-transparent text-[var(--cyber-primary)] border-[rgba(0,255,255,0.40)] shadow-[0_0_14px_rgba(0,255,255,0.10)]'
            : 'bg-transparent text-[var(--cyber-text-muted)] border-[rgba(255,255,255,0.14)]'}`}>

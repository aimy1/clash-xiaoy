...
export const NodeCard = memo(function NodeCard({ node, now, disabled, style, }) {
    ...
    return (<Paper elevation={0} style={style} className={cn('relative flex flex-col justify-between p-3 cursor-pointer overflow-hidden cyber-glass ios-motion ios-pressable', checked ? 'cyber-box-glow' : undefined, styles.Card)} sx={{
      ...
      <Box width="100%" display="flex" justifyContent="space-between" alignItems="center">
        ...
        <DelayChip className={styles.DelayChip} delay={delay} onClick={handleDelayClick}/>
      </Box>
      
      {/* Signal Bar Visual */}
      <div className="mt-2 h-1 w-full rounded-full overflow-hidden" style={{ backgroundColor: 'var(--bg-active)' }}>
        <div className={cn('h-full ios-signal-fill', delay > 0 ? undefined : 'opacity-0')} style={{
            width: delay > 0 ? '100%' : '0%',

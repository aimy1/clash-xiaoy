...
function AnimatedOutlet({ ref, className, ...others }) {
    ...
    return (<motion.div ref={ref} className={className} {...others}>
...
export default function PageTransition({ className }) {
    const { value: lightenAnimationEffects } = useSetting('lighten_animation_effects');
    const matches = useMatches();
    ...
    return (<AnimatePresence mode="popLayout" initial={false}>
      <AnimatedOutlet className={cn('page-transition', className)} key={nextMatch ? nextMatch.id : ''} layout layoutId={nextMatch.id} variants={variants} initial="initial" animate="visible" exit="hidden"/>
    </AnimatePresence>);
}

...
export default function ConnectionTotal() {
    ...
    const downloadHighlightTimerRef = useRef(null);
    const uploadHighlightTimerRef = useRef(null);
    useEffect(() => {
        if (latestClashConnections?.downloadTotal &&
    ...
    useEffect(() => {
        if (latestClashConnections?.uploadTotal &&
    ...
    if (isLoading || !latestClashConnections) {
        ...
        <Paper elevation={0} className="flex min-h-8 items-center justify-center gap-1 px-2" sx={{
                borderRadius: '1em',
            }}>
    ...
    return (<div className="flex gap-2">
      <Paper elevation={0} className="flex min-h-8 items-center justify-center gap-1 px-2" sx={{
        ...
        <Download className="scale-75" sx={((theme) => ({
            color: darken(theme.vars.palette.primary.main, downloadHighlight ? 0.9 : 0.3),
            ...theme.applyStyles('dark', {
                color: lighten(theme.vars.palette.primary.main, downloadHighlight ? 0.2 : 0.9),
            }),
        }))}/>{' '}
      ...
      <Paper elevation={0} className="flex min-h-8 items-center justify-center gap-1 px-2" sx={{
        ...
        <Upload className="scale-75" sx={((theme) => ({
            color: darken(theme.vars.palette.primary.main, uploadHighlight ? 0.9 : 0.3),
            ...theme.applyStyles('dark', {
                color: lighten(theme.vars.palette.primary.main, downloadHighlight ? 0.2 : 0.9),
            }),
        }))}/>{' '}

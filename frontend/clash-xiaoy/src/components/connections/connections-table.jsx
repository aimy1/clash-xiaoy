...
export const ConnectionsTable = ({ searchTerm }) => {
    const { t, i18n } = useTranslation();
    const { query: { data: clashConnections, isLoading }, } = useClashConnections();
    const historyMessage = useRef(null);
    const connectionsMessage = useMemo(() => {
        ...
        const filteredConnections = searchTerm
            ? result.connections?.filter((connection) => containsSearchTerm(connection, searchTerm))
            : result.connections;
        filteredConnections?.forEach((connection) => {
            const previousConnection = historyMessage.current?.connections.find((history) => history.id === connection.id);
    ...
    const filteredColumns = useMemo(() => columns
        .filter((column) => tableColsOrder.find((o) => o[0] === column.id)?.[1] ?? true)
        .sort((a, b) => {
    ...
    const columnOrder = useMemo(() => filteredColumns.map((column) => column.id), [filteredColumns]);
    ...
    const [connectionDetailDialogOpen, setConnectionDetailDialogOpen] = useState(false);
    const [connectioNDetailDialogItem, setConnectionDetailDialogItem] = useState(undefined);
    ...
    return connectionsMessage?.connections.length ? (<>
      <ConnectionDetailDialog item={connectioNDetailDialogItem} open={connectionDetailDialogOpen} onClose={() => setConnectionDetailDialogOpen(false)}/>

      <MaterialReactTable table={table}/>
    </>) : (<ContentDisplay className="!absolute !h-full !w-full" message={t('No Connections')}/>);
};

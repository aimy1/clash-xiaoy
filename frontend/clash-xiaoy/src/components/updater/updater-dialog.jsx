...
export default function UpdaterDialog({ open, update, onClose, ...others }) {
    ...
    const progress = contentDownloaded && contentLength
    ...
    const date = update.date ||
        (typeof update.rawJson.pub_date === 'string'
            ...
            : undefined);
    console.info(date);
    const onDownloadEvent = useCallback((e) => {
        switch (e.event) {
            case 'Started':
    ...
    return (<BaseDialog {...others} title={t('updater.title')} open={open} onClose={() => {
            setUpdaterIgnore(update.version); // TODO: control this behavior
            onClose?.();
        }} onOk={handleUpdate} loading={pending} close={t('updater.close')} ok={t('updater.update')} divider>
      <div className={cn('xs:min-w-[90vw] sm:min-w-[55vw] md:min-w-[33.3vw]', styles.UpdaterDialog)}>
        ...
        <div className={cn('h-[50vh] overflow-y-auto p-4', styles.MarkdownContent)}>
        ...
        {pending && (<div className="mt-2 flex items-center gap-2">
            <LinearProgress className="flex-1" variant="determinate" value={progress}/>

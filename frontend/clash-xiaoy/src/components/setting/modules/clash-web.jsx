 ...
 */
export const renderChip = (string, labels) => {
    return string.split(/(%[^&?]+)/).map((part, index) => {
        if (part.startsWith('%')) {
            ...
            return (<Chip sx={{
                ...
                }} key={index} size="small" label={label}/>);
        }
        else {
            return part;
        }
 ...
 * @copyright LibNyanpasu org. 2024
 */
export const extractServer = (string) => {
    if (!string) {
 ...
 * @copyright LibNyanpasu org. 2024
 */
export const openWebUrl = (string, labels) => {
    let url = '';
    for (const key in labels) {
        const regex = new RegExp(`%${key}`, 'g');
        url = string.replace(regex, labels[key]);
    }
    openThat(url);
};
/**
 ...
 * @copyright LibNyanpasu org. 2024
 */
export const ClashWebItem = ({ label, onOpen, onDelete, onEdit, }) => {
    return (<Item>
      <Marquee>

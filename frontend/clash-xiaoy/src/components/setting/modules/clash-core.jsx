...
import { inspectUpdater, useClashCores, } from '@nyanpasu/interface';
import { alpha, cleanDeepClickEvent, cn } from '@nyanpasu/ui';
export const getImage = (core) => {
    switch (core) {
        case 'mihomo':
        case 'mihomo-alpha': {
            return ClashMeta;
...
const calcProgress = (data) => {
    return ((Number(data?.downloader?.downloaded) / Number(data?.downloader?.total)) *
        100);
};
const CardProgress = ({ data, show, }) => {
    const parsedState = () => {
    ...
    return (<Box component={motion.div} className={cn('absolute top-0 left-0 z-10 h-full w-full rounded-2xl backdrop-blur', 'flex flex-col items-center justify-center gap-2')} sx={(theme) => ({
            backgroundColor: alpha(theme.vars.palette.primary.main, 0.3),
        })} animate={show ? 'open' : 'closed'} initial={{ opacity: 0 }} variants={{
            open: {
                opacity: 1,
                display: 'flex',
            },
      ...
      <Box className="absolute left-0 h-full rounded-2xl transition-all" sx={(theme) => ({
            backgroundColor: alpha(theme.vars.palette.primary.main, 0.3),
 ...
 */
export const ClashCoreItem = ({ selected, data, core, onClick, }) => {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                const interval = setInterval(async () => {
                    ...
                    setUpdater(result);
                    if (isObject(result.downloader.state) &&
                        Object.prototype.hasOwnProperty.call(result.downloader.state, 'failed')) {
    ...
    return (<ListItem sx={{ pl: 0, pr: 0 }}>
      <ListItemButton className="!relative !p-0" sx={(theme) => ({
            borderRadius: '16px',
        ...
        <div className="flex w-full items-center gap-2 p-4">
          ...
          <div className="flex-1">
            <div className="truncate font-bold">
              ...
              {haveNewVersion && (<FiberManualRecord sx={(theme) => ({
                height: 10,
            ...
            {haveNewVersion && (<div className="truncate text-sm">New: {data.latestVersion}</div>)}
          </div>

          {haveNewVersion && (<Tooltip title={t('Update Core')}>
              <Button variant="text" className="!size-8 !min-w-0" loading={downloadState} onClick={(e) => {

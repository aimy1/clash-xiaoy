...
function CopyToClipboardButton({ onClick }) {
    const { t } = useTranslation();
    return (<Tooltip title={t('Copy to clipboard')} placement="top" slotProps={{
            popper: {
                modifiers: [
                    {
                        name: 'offset',
      ...
      <IconButton size="small" className="!absolute top-1 right-1" onClick={onClick}>
        <ContentPasteIcon fontSize="small" color="primary"/>
      </IconButton>
    </Tooltip>);
}
// TODO: maybe support more commands prompt?
export default function ServerManualPromptDialog({ open, onClose, operation, ...props }) {
    const { t } = useTranslation();
    const { mode } = useColorScheme();
    const { data: serviceInstallPrompt, error } = useSWR(operation === 'install' ? '/service_install_prompt' : null, getServiceInstallPrompt);
    ...
    const handleCopyToClipboard = useCallback(() => {
        if (commands) {
            ...
            navigator.clipboard
                ...
                console.log('copied');
                notification({
                    title: `clash-xiaoy - ${t('Service Manual Tips')}`,
    ...
    return (<BaseDialog title={t('Service Manual Tips')} open={open} onClose={onClose} {...props}>
      <div className="grid gap-3">
        ...
        {!!codes && (<div className="relative">
            <div className={cn('rounded-sm md:max-w-[80vw] lg:max-w-[60vw] xl:max-w-[50vw]', mode === 'dark' && styles.dark, styles.prompt)} dangerouslySetInnerHTML={{
                __html: codes,
            }}/>
...
export function ServerManualPromptDialogWrapper() {
    ...
    return (<ServerManualPromptDialog open={!!prompt} onClose={() => setPrompt(null)} operation={prompt}/>);
}
export function useServerManualPromptDialog() {
    const setPrompt = useSetAtom(serviceManualPromptDialogAtom);
    return {
        show: (prompt) => setPrompt(prompt),

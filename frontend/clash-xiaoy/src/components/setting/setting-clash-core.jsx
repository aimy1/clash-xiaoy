...
import { ClashCores, useClashConnections, useClashCores, useClashVersion, useSetting, } from '@nyanpasu/interface';
...
export const SettingClashCore = () => {
    ...
    const { query: clashCores, upsert: switchCore, restartSidecar, fetchRemote, } = useClashCores();
    ...
    const changeClashCore = useLockFn(async (core) => {
        try {
            loading.mask = true;
            try {
            ...
            message(t('Successfully switched to the clash core', {
            ...
            message(t('Failed to switch. You could see the details in the log', {
    ...
    const handleRestart = async () => {
        try {
            ...
            message(t('Failed to restart. You could see the details in the log') +
                formatError(e), {
                kind: 'error',
                title: t('Error'),
            });
    ...
    return (<BaseCard label={t('Clash Core')} loading={loading.mask} labelChildren={<span>{version}</span>}>
      <List disablePadding>
        {clashCores.data &&
            Object.entries(clashCores.data).map(([core, item]) => {
                const show = expand || core === currentCore;
                return (<motion.div key={item.name} animate={show ? 'open' : 'closed'} variants={{
                        open: {
                            height: 'auto',
                            opacity: 1,
                            scale: 1,
                ...
                <ClashCoreItem data={item} core={core} selected={core === currentCore} onClick={() => changeClashCore(core)}/>
              </motion.div>);
            })}
        ...
        <ListItem sx={{
          ...
          <Box display="flex" gap={1}>
            ...
            {OS !== 'linux' && (<LoadingButton variant="contained" loading={fetchRemote.isPending} onClick={handleCheckUpdates}>
                {t('Check Updates')}
              </LoadingButton>)}
          </Box>

...
const ClashPort = () => {
    ...
    return (<NumberItem label={t('Mixed Port')} value={port} checkEvent={(input) => input > 65535 || input < 1} checkLabel="Port must be between 1 and 65535." onApply={async (value) => {
...
const RandomPort = () => {
    ...
    return (<SwitchItem label={t('Random Port')} checked={value || false} onChange={handleRandomPort}/>);
};
export const SettingClashPort = () => {
    const { t } = useTranslation();
    return (<BaseCard label={t('Clash Port')}>
      <List disablePadding>
        <ClashPort />

...
const FieldsControl = ({ label, fields, enabledFields, onChange, }) => {
    const [open, setOpen] = useState(false);
    // clash-xiaoy Control Fields object key
    const disabled = label === 'default' || label === 'handle';
    const showFields = disabled
    ...
    const Item = () => {
        return Object.entries(fields).map(([fKey, fValue], fIndex) => {
            ...
            return (<LabelSwitch key={fIndex} label={fKey} url={fValue} disabled={disabled} checked={disabled ? true : checked} onChange={onChange ? () => onChange(fKey) : undefined}/>);
        });
    };
    return (<>
      <ClashFieldItem label={label} fields={showFields} onClick={() => setOpen(true)}/>

      <BaseDialog title={label} open={open} close="Close" onClose={() => setOpen(false)} divider contentStyle={{ overflow: 'auto' }}>
        <Box display="flex" flexDirection="column" gap={1}>
          {disabled && <Typography>clash-xiaoy Control Fields.</Typography>}
...
const ClashFieldSwitch = () => {
    ...
    return (<LabelSwitch label={t('Enable Clash Fields Filter')} checked={Boolean(value)} onChange={() => upsert(!value)}/>);
};
export const SettingClashField = () => {
    ...
    return (<BaseCard label={t('Clash Field')}>
      <Box sx={{ pt: 1, pb: 2 }}>
        <ClashFieldSwitch />
      </Box>
      ...
      <Grid container spacing={2}>
        {Object.entries(CLASH_FIELD).map(([key, value], index) => {
            ...
            return (<FieldsControl key={index} label={key} fields={value} enabledFields={filtered} onChange={updateFiled}/>);
        })}
      </Grid>
    </BaseCard>);
};

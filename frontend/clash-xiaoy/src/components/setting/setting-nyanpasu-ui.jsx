...
const LanguageSwitch = () => {
    ...
    return (<MenuItem label={t('Language')} selectSx={commonSx} options={languageOptions} selected={language.value || 'en'} onSelected={(value) => language.upsert(value)}/>);
};
const ThemeSwitch = () => {
    const { t } = useTranslation();
    const themeOptions = {
    ...
    return (<MenuItem label={t('Theme Mode')} selectSx={commonSx} options={themeOptions} selected={themeMode.value || 'system'} onSelected={(value) => themeMode.upsert(value)}/>);
};
const ThemeColor = () => {
    const { t } = useTranslation();
    ...
    return (<>
      <ListItem sx={{ pl: 0, pr: 0 }}>
        ...
        <MuiColorInput size="small" sx={{
            ...
            '& .MuiColorInput-Button': {
                ...
                boxShadow: '0 1px 0 rgba(255, 255, 255, 0.12) inset, var(--shadow-card-active)',
                transition: 'transform var(--motion-duration-medium) var(--motion-ease-ios), box-shadow var(--motion-duration-medium) var(--motion-ease-ios)',
            },
            '& .MuiColorInput-Button:hover': {
                transform: 'translateY(-1px)',
                boxShadow: '0 1px 0 rgba(255, 255, 255, 0.14) inset, var(--shadow-card-hover)',
            },
        ...
        }} value={value ?? DEFAULT_COLOR} isAlphaHidden format="hex" onBlur={() => {
      ...
      <Expand open={(theme.value || DEFAULT_COLOR) !== value}>
        <div className="flex justify-end">
          <Button variant="contained" startIcon={<Done />} onClick={() => {
            if (isHexColor(value)) {
                theme.upsert(value);
            }
            else {
...
export const SettingNyanpasuUI = () => {
    ...
    return (<BaseCard label={t('User Interface')}>
      <List disablePadding>
        <LanguageSwitch />
        ...
        <SwitchItem label={t('Icon Navigation Bar')} checked={onlyIcon} onChange={() => setOnlyIcon(!onlyIcon)}/>

...
import { openUWPTool, useClashConfig, useRuntimeProfile, useSetting, } from '@nyanpasu/interface';
...
const AllowLan = () => {
    ...
    return (<SwitchItem label={t('Allow LAN')} checked={value} onChange={async () => {
...
const IPv6 = () => {
    ...
    return (<SwitchItem label={t('IPv6')} checked={value} onChange={async () => {
            await upsert.mutateAsync({
                ipv6: !value,
            });
        }}/>);
...
const TunStack = () => {
    ...
    return (<MenuItem label={t('TUN Stack')} options={tunStackOptions} selected={selected} onSelected={async (value) => {
            try {
                await upsertTunStack(value);
                if (enableTun) {
...
const LogLevel = () => {
    ...
    return (<MenuItem label={t('Log Level')} options={options} selected={value ?? 'debug'} onSelected={async (value) => {
            await upsert.mutateAsync({
                'log-level': value,
            });
        }}/>);
...
export const SettingClashBase = () => {
    const { t } = useTranslation();
    const [coreType] = useCoreType();
    return (<BaseCard label={t('Clash Setting')}>
      <List disablePadding>
        <AllowLan />

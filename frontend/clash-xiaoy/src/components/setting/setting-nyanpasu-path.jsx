...
import { collectLogs, openAppConfigDir, openAppDataDir, openCoreDir, openLogsDir, restartApplication, setCustomAppDir, } from '@nyanpasu/interface';
...
export const SettingNyanpasuPath = () => {
    ...
    return (<BaseCard label={t('Path Config')}>
      <Grid container alignItems="stretch" spacing={2}>
        {gridLists.map(({ label, onClick }) => (<Grid key={label} size={{
                xs: 6,
                xl: 3,
            }}>
            <PaperButton label={label} onClick={onClick} sxPaper={{ height: '100%' }} sxButton={{ height: '100%' }}/>
          </Grid>))}
      </Grid>
    </BaseCard>);
};

...
const AddRecordButton = ({ onClick }) => {
    ...
    return (<Tooltip title={t('New Item')}>
...
export const SettingClashWeb = () => {
    ...
    const deleteItem = useLockFn(async (index) => {
        await upsert(value ? value.slice(0, index).concat(value.slice(index + 1)) : null);
    });
    ...
    return (<>
      <BaseCard label={t('Web UI')} labelChildren={<AddRecordButton onClick={() => {
                setEditString('');
                setEditIndex(null);
                setOpen(true);
            }}/>}>
        {value && (<Grid container sx={{ mt: 1 }} spacing={2}>
            {value.map((item, index) => {
                return (<Grid key={index} size={{
                        xs: 12,
                        xl: 6,
                    }}>
                  <ClashWebItem label={renderChip(item, labels)} onOpen={() => openWebUrl(item, labels)} onEdit={() => {
                        setEditIndex(index);
                        setEditString(item);
                        setOpen(true);
      ...
      <BaseDialog title={editIndex != null ? t('Edit Item') : t('New Item')} open={open} onClose={() => {
            setOpen(false);
            setEditIndex(null);
        }} onOk={() => {
            updateItem();
            ...
            setEditIndex(null);
            setEditString('');
        }} ok={t('Ok')} close={t('Close')} contentStyle={{ overflow: editString ? 'auto' : 'hidden' }} divider>
        <Box display="flex" flexDirection="column" gap={1}>
          <Typography variant="h5">{t('Input')}</Typography>

          <TextField sx={{ width: '100%' }} value={editString} variant="outlined" multiline placeholder={t(`Support %host %port, and %secret`)} onChange={(e) => setEditString(e.target.value)}/>

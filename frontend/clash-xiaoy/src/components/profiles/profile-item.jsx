...
import { FiberManualRecord, FilterDrama, InsertDriveFile, Menu as MenuIcon, Terminal, Update, } from '@mui/icons-material';
import { Badge, Button, Chip, LinearProgress, Menu, MenuItem, Paper, Tooltip, } from '@mui/material';
import { useClashConnections, useProfile, } from '@nyanpasu/interface';
import { alpha, cleanDeepClickEvent, cn } from '@nyanpasu/ui';
import { ProfileDialog } from './profile-dialog';
import { GlobalUpdatePendingContext } from './provider';
export const ProfileItem = memo(function ProfileItem({ item, selected, onClickChains, chainsSelected, maxLogLevelTriggered, }) {
    ...
    const handleUpdate = useLockFn(async (proxy) => {
    ...
    return (<>
      <Paper className={cn("relative transition-all cyber-glass", selected && "cyber-box-glow border-neon-primary")} sx={[
            {
                borderRadius: 6,
        ...
        <div className="flex cursor-pointer flex-col gap-4 p-5" onClick={handleSelect}>
          <div className="flex items-center justify-between gap-2">
            <Tooltip title={item.url}>
              <Chip className="!pr-2 !pl-2 font-bold" avatar={<IconComponent className="!size-5" color="primary"/>} label={isRemote ? t('Remote') : t('Local')} sx={{
            ...
            {selected && (<FiberManualRecord className="top-0 mr-auto !size-3 animate-pulse" sx={(theme) => ({
                fill: theme.vars.palette.success.main,
                filter: 'drop-shadow(0 0 5px currentColor)',
            })}/>)}

            <TextCarousel className="flex h-6 w-30 items-center" nodes={[
            !!item.updated && (<TimeSpan ts={item.updated} k="Subscription Updated At"/>),
            !!item.extra?.expire && (<TimeSpan ts={item.extra.expire} k="Subscription Expires In"/>),
        ]}/>
          </div>
          ...
          <div className={cn('flex items-center justify-between gap-4', !isRemote && 'invisible')}>
          ...
          <div className="flex justify-end gap-2">
            <Badge variant="dot" color={maxLogLevelTriggered?.current === 'error'
            ? 'error'
            : maxLogLevelTriggered?.current === 'warn'
                ? 'warning'
                : 'primary'} invisible={!selected || !maxLogLevelTriggered?.current}>
              <Button className="!mr-auto" size="small" variant={chainsSelected ? 'contained' : 'outlined'} startIcon={<Terminal />} onClick={(e) => {
            cleanDeepClickEvent(e);
            onClickChains(item);
        }}>
                {t('Proxy Chains')}
            ...
            <Tooltip title={t('Update')}>
              <Button size="small" variant="outlined" className="!size-8 !min-w-0" onClick={(e) => {
            cleanDeepClickEvent(e);
            menuMapping.Update();
            ...
            <Tooltip title={t('Menu')}>
              <Button size="small" variant="contained" className="!size-8 !min-w-0" onClick={(e) => {
            cleanDeepClickEvent(e);
        ...
        <motion.div className={cn('absolute top-0 left-0 h-full w-full', 'flex-col items-center justify-center gap-4', 'text-shadow-xl rounded-3xl font-bold backdrop-blur')} initial={{ opacity: 0, display: 'none' }} animate={loading.card ? 'show' : 'hidden'} variants={{
      ...
      {MenuComp}
      <ProfileDialog open={open} onClose={() => setOpen(false)} profile={item}/>
    </>);
});
function TimeSpan({ ts, k }) {
    const time = dayjs(ts * 1000);
    const { t } = useTranslation();
    return (<Tooltip title={time.format('YYYY/MM/DD HH:mm:ss')}>
...
function TextCarousel(props) {
    ...
    const nodes = useMemo(() => props.nodes.filter((item) => !!item), [props.nodes]);
    ...
    return (<div className={cn('overflow-hidden', props.className)} onClick={() => nextNode()}>
      <AnimatePresence mode="wait">
        {nodes.map((node, i) => i === index && (<motion.div className="h-full w-full" key={index} initial={{ y: 40, opacity: 0, scale: 0.8 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: -40, opacity: 0, scale: 0.8 }}>
                {node}
              </motion.div>))}
      </AnimatePresence>
    </div>);

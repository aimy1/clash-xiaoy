...
export const SideChain = ({ onChainEdit }) => {
    ...
    const currentProfile = useMemo(() => {
        return clash?.find((item) => item.uid === currentProfileUid);
    ...
    const handleChainClick = useLockFn(async (uid) => {
    ...
    const reorderValues = useMemo(() => filteredChains?.map((item) => item.uid) || [], [filteredChains]);
    return (<div className="h-full overflow-auto !pr-2 !pl-2">
      <Reorder.Group axis="y" values={reorderValues} onReorder={(values) => {
            const profileUids = clash?.map((item) => item.uid) || [];
            sort.mutate([...profileUids, ...values]);
        }} layoutScroll style={{ overflowY: 'scroll' }}>
        {filteredChains?.map((item, index) => {
            ...
            const usedInCurrentProfile = currentProfile?.chain?.includes(item.uid) ?? false;
            return (<ChainItem key={index} item={item} selected={selected} context={{
                    global: usedInGlobal,
                    scoped: usedInCurrentProfile,
                }} onClick={async () => await handleChainClick(item.uid)} onChainEdit={() => onChainEdit(item)}/>);
        })}
      </Reorder.Group>

      <ListItemButton className="!mt-2 !mb-2 flex justify-center gap-2" sx={(theme) => ({

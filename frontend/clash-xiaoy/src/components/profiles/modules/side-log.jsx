...
const LogListItem = memo(function LogListItem({ name, item, showDivider, }) {
    return (<>
      {showDivider && <Divider />}
...
export const SideLog = ({ className }) => {
    ...
    return (<div className={cn('w-full', className)}>
      ...
      <VList className="flex flex-col gap-2 overflow-auto p-2 select-text">
        {currentLogs ? (Object.entries(currentLogs).map(([uid, content]) => {
            return content?.map((item, index) => {
                const name = profiles?.find((script) => script.uid === uid)?.name;
                return (<LogListItem key={uid + index} name={name} item={item} showDivider={index !== 0}/>);
            });

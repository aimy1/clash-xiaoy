...
export const ChainItem = memo(function ChainItem({ item, selected, context, onClick, onChainEdit, }) {
    ...
    onPointerDown={(e) => {
        ...
        <ListItemButton className="!mt-2 !mb-2 !flex !justify-between gap-2" sx={[
            {
                borderRadius: 4,
            },
            (theme) => ({
        ...
        ]} onClick={handleClick} disabled={isPending}>
          <div className="truncate py-1">
            <span>{item.name}</span>
            <div className="mt-1 flex gap-1">
              {context?.global && (<span className="rounded bg-blue-500 px-1 py-0.5 text-xs text-white">
                  G
                </span>)}
              {context?.scoped && (<span className="rounded bg-green-500 px-1 py-0.5 text-xs text-white">
                  S
                </span>)}
            </div>
          </div>

          <Button size="small" color="primary" className="!size-8 !min-w-0" loading={isPending} onClick={(e) => {
            cleanDeepClickEvent(e);

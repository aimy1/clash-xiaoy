...
import { useProfile, useProfileContent, useRuntimeProfile, } from '@nyanpasu/interface';
import { BaseDialog, cn } from '@nyanpasu/ui';
const MonacoDiffEditor = lazy(() => import('./profile-monaco-diff-viewer'));
export default function RuntimeConfigDiffDialog({ open, onClose, }) {
    ...
    return (<BaseDialog title={t('Runtime Config')} open={open} onClose={onClose}>
      <div className="xs:w-[95vw] h-full w-[80vw] px-4">
        <div className={cn('items-center justify-between px-5 pb-2', loaded ? 'flex' : 'hidden')}>
        ...
        <div className="h-[75vh] w-full">
          <Suspense fallback={null}>
            {loaded && (<MonacoDiffEditor language="yaml" theme={mode === 'light' ? 'vs' : 'vs-dark'} original={contentFn.query.data} originalModelPath={originalModelPath} modified={runtimeProfile.data} modifiedModelPath={modifiedModelPath} options={{

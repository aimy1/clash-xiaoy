...
export const AppDrawer = () => {
    ...
    const DrawerTitle = () => {
        return (<div className={cn('fixed z-10 flex items-center gap-2', OS === 'macos' ? 'top-3 left-24' : 'top-1.5 left-4')} data-tauri-drag-region>
        <IconButton className="!size-8 !min-w-0" sx={[
                (theme) => ({
    ...
    return (<>
      <DrawerTitle />
      <Backdrop className={cn('z-20', OS !== 'linux' && 'backdrop-blur-xl')} sx={(OS === 'linux'
            ? {
        ...
        <AnimatePresence initial={false}>
          <div className="h-full w-full">
            <motion.div className="h-full" animate={open ? 'open' : 'closed'} variants={{
            open: {
                x: 0,
            },
            closed: {

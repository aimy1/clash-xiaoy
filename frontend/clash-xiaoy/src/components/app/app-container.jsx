...
export const AppContainer = ({ children, isDrawer, }) => {
    ...
    return (<Paper square elevation={0} className={cn(styles.layout, 'gap-4')} onPointerDown={(e) => {
      ...
      {!isDrawer && (<div className={cn(onlyIcon ? 'w-24' : 'w-64')}>
          <DrawerContent className="rounded-3xl border border-[var(--cyber-glass-border)]" data-tauri-drag-region onlyIcon={onlyIcon}/>
        </div>)}
      ...
      <div className={cn(styles.container, 'cyber-glass rounded-3xl border border-[var(--cyber-glass-border)] bg-scanline animate-pulse-neon flex flex-col overflow-hidden')}>
        ...
        <div className={cn(OS === 'macos' ? 'h-[2.75rem]' : 'h-12', 'shrink-0 border-b border-[var(--cyber-glass-border)] bg-[rgba(0,0,0,0.2)]')}>
          <TopBar />
        </div>

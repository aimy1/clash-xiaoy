...
export const RouteListItem = ({ name, path, icon, onlyIcon, }) => {
    const { t } = useTranslation();
    const location = useLocation();
    const match = location.pathname === path;
    const navigate = useNavigate();
    const { value: language } = useSetting('language');
    const listItemButton = (<ListItemButton className={cn(onlyIcon ? '!mx-auto !size-12 !rounded-xl !p-0 !min-h-0' : '!rounded-xl !py-2.5 !px-4', match && 'cyber-box-glow', 'ios-motion')} sx={[
            (theme) => ({
                ...
                transition: 'transform var(--motion-duration-medium) var(--motion-ease-ios), background-color var(--motion-duration-medium) var(--motion-ease-ios), box-shadow var(--motion-duration-medium) var(--motion-ease-ios)',
      ...
      {!onlyIcon && (<Box className={cn('w-full pt-1 pb-1 text-nowrap', language && languageQuirks[language].drawer.itemClassNames)} sx={(theme) => ({

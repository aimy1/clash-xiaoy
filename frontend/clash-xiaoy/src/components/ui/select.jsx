...
export const selectValueVariants = cva('pointer-events-none transition-[margin] duration-200', {
    variants: {
        variant: {
            filled: '',
            outlined: '',
...
export const selectValuePlaceholderFieldsetVariants = cva('pointer-events-none', {
    variants: {
        variant: {
...
export const SelectLine = ({ className, ...props }) => {
    const { variant } = useSelectContext();
    return (<div className={cn(selectLineVariants({
            variant,
        }), className)} {...props}/>);
};
export const Select = ({ onValueChange, variant, open: inputOpen, defaultOpen, onOpenChange, ...props }) => {
    const [open, setOpen] = useControllableState({
        prop: inputOpen,
        defaultProp: defaultOpen ?? false,
        onChange: onOpenChange,
    });
    const [haveValue, setHaveValue] = useState(Boolean(props.value || props.defaultValue));
    ...
    return (<SelectContext.Provider value={{
      ...
      <SelectPrimitive.Root open={open} onOpenChange={setOpen} onValueChange={chains(handleOnChange, onValueChange)} {...props}/>
    </SelectContext.Provider>);
};
export const SelectValue = ({ className, placeholder, ...props }) => {
    const { haveValue, open, variant } = useSelectContext();
    return (<>
      <div className={cn(selectValueVariants({
            variant,
            haveValue,
        }), className)}>
      ...
      <fieldset className={cn(selectValuePlaceholderFieldsetVariants({
            variant,
        }))}>
        <legend className={cn(selectValuePlaceholderLegendVariants({
            variant,
      ...
      <div className={cn(selectValuePlaceholderVariants({
            variant,
...
export const SelectLabel = ({ className, ...props }) => {
    return (<SelectPrimitive.Label className={cn('flex h-12 cursor-default items-center justify-between gap-2 p-4 outline-hidden', 'text-primary dark:text-inverse-primary', className)} {...props}/>);
};
export const SelectTrigger = ({ className, children, ...props }) => {
    const { variant } = useSelectContext();
    return (<SelectPrimitive.Trigger className={cn(selectTriggerVariants({
            variant,
        }), className)} {...props}>
      {children}
...
export const SelectIcon = ({ asChild, children, className, ...props }) => {
    return (<SelectPrimitive.Icon className={cn('absolute right-4', className)} asChild {...props}>
...
export const SelectContent = ({ className, children, ...props }) => {
    ...
    return (<AnimatePresence initial={false}>
      {open && (<SelectPrimitive.Portal>
          <SelectPrimitive.Content {...props} position="popper" asChild>
            <motion.div className={cn(selectContentVariants({
                variant,
            }), className)} style={{
                width: 'var(--radix-popper-anchor-width)',
                maxHeight: 'var(--radix-popper-available-height)',
            }} initial={{ opacity: 0, scaleY: 0.9, transformOrigin: 'top' }} animate={{ opacity: 1, scaleY: 1, transformOrigin: 'top' }} exit={{ opacity: 0, scaleY: 0.9, transformOrigin: 'top' }} transition={{
                type: 'spring',
                bounce: 0,
                duration: 0.35,
            }}>
...
export const SelectItem = ({ className, children, ...props }) => {
    return (<SelectPrimitive.Item className={cn('flex h-12 cursor-default items-center justify-between gap-2 p-4 outline-hidden', 'cursor-pointer', 'hover:bg-surface-variant data-[state=checked]:bg-primary-container', 'dark:hover:bg-surface-variant dark:data-[state=checked]:bg-primary-container', className)} {...props}>

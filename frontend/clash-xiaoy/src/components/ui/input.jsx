...
export const InputContainer = ({ className, ...props }) => {
    const { variant } = useInputContext();
    return (<div className={cn(inputContainerVariants({
            variant,
        }), className)} {...props}/>);
};
export const InputLine = ({ className, ...props }) => {
    const { variant } = useInputContext();
    return (<div className={cn(inputLineVariants({
            variant,
        }), className)} {...props}/>);
};
export const Input = ({ variant, className, label, children, onChange, ...props }) => {
    ...
    return (<InputContext.Provider value={{
      ...
      <InputContainer>
        <input className={cn(inputVariants({
            ...
            haveValue,
            haveLabel,
        }), className)} autoComplete="off" autoCapitalize="off" autoCorrect="off" spellCheck={false} onChange={handleChange} {...props}/>

        {label && (<>
            <fieldset className={cn(inputLabelFieldsetVariants({
                variant,
            }))}>
              <legend className={cn(inputLabelLegendVariants({
                variant,
                haveValue,
            }))}>
                {label}
...
export const InputLabel = ({ className, ...props }) => {
    ...
    return (<label className={cn(inputLabelVariants({
            variant,
            focus: haveValue,
        }))} {...props}/>);
};
InputLabel.displayName = 'InputLabel';
export const NumericInput = ({ label, variant, className, onChange, value, defaultValue, min, max, step = 1, decimalScale, allowNegative = true, ...props }) => {
    ...
    const validateAndFormatValue = (numValue) => {
    ...
    return (<Input label={label} variant={variant} className={className} value={inputValue} onChange={handleChange} onBlur={handleBlur} type="text" inputMode="decimal" {...props}/>);
};

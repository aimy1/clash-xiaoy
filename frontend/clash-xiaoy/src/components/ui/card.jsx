...
export const cardHeaderVariants = cva(['flex items-center gap-4 text-xl', 'px-4'], {
    variants: {
        variant: {
...
export const cardFooterVariants = cva(['flex flex-row-reverse items-center gap-4', 'px-2'], {
    variants: {
        variant: {
            basic: 'border-surface-variant dark:border-surface-variant',
            raised: 'border-inverse-primary dark:border-primary-container',
...
export const Card = ({ variant, divider, asChild, className, ...props }) => {
    const Comp = asChild ? Slot : 'div';
    return (<CardContext.Provider value={{
            variant,
            divider,
        }}>
      <Comp className={cn(cardVariants({
            variant,
        }), className)} {...props}/>
...
export const CardHeader = ({ divider, variant, className, ...props }) => {
    const context = useCardContext();
    return (<div className={cn(cardHeaderVariants({
            divider: context?.divider ?? divider,
            variant: context?.variant ?? variant,
        }), className)} {...props}/>);
};
export const CardFooter = ({ divider, variant, className, ...props }) => {
    const context = useCardContext();
    return (<div className={cn(cardFooterVariants({
            divider: context?.divider ?? divider,
            variant: context?.variant ?? variant,
        }), className)} {...props}/>);
};

...
export const buttonVariants = cva([
    ...
    compoundVariants: [
        ...
        },
        {
            variant: 'fab',
            disabled: true,
            className: 'bg-gray-900/20 text-gray-900/40 hover:brightness-100 hover:shadow-container-xl',
        },
        {
            icon: true,
            className: 'w-10',
...
const LazyRipple = lazy(() => import('./ripple').then((mod) => ({ default: mod.Ripple })));
export const Button = ({ loading, asChild, variant, disabled, icon, className, children, onClick, ...props }) => {
    ...
    return (<Comp className={cn(buttonVariants({
            ...
            disabled,
            icon,
        }), className)} onClick={handleClick} data-loading={String(Boolean(loading))} {...props}>
      <Slottable>{children}</Slottable>

      <AnimatePresence initial={false}>
        {loading && (<motion.span className={cn('absolute inset-0 z-10 flex h-full w-full cursor-wait items-center justify-center', 'bg-inherit-allow-fallback')} data-slot="button-loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <CircularProgress className="size-3/5" indeterminate/>
          </motion.span>)}
      </AnimatePresence>

      <Suspense>
        {ripple && !loading && !disabled && (<LazyRipple ripples={ripple.ripples} onClear={handleClear}/>)}
      </Suspense>
    </Comp>);
};

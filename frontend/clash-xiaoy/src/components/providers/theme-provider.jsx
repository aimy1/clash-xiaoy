...
const THEME_PALETTE_KEY = 'theme-palette-v3';
const THEME_CSS_VARS_KEY = 'theme-css-vars-v3';
const intToRgba = (int, alpha) => {
    const r = (int >> 16) & 0xff;
    const g = (int >> 8) & 0xff;
    const b = int & 0xff;
    return `rgba(${r}, ${g}, ${b}, ${alpha})`;
};
const generateThemeCssVars = ({ schemes }) => {
...
const changeHtmlThemeMode = (mode) => {
...
export function useExperimentalThemeContext() {
    ...
    if (!context) {
        throw new Error('useExperimentalThemeContext must be used within a ExperimentalThemeProvider');
    }
    return context;
}
export function ExperimentalThemeProvider({ children }) {
    ...
    const [resolvedThemeMode, setResolvedThemeMode] = useState(themeMode.value === ThemeMode.SYSTEM
    ...
    useEffect(() => {
        if (themeMode.value === ThemeMode.SYSTEM) {
            ...
            setResolvedThemeMode(themeMode.value);
        }
    }, [themeMode.value]);
    const isUsingDefault = !themeColorValue || themeColorValue.toLowerCase() === OLD_DEFAULT_COLOR.toLowerCase();
    const [cachedThemePalette, setCachedThemePalette] = useLocalStorage(THEME_PALETTE_KEY, themeFromSourceColor(
    // use default color if theme color is not set or is old default
    argbFromHex(isUsingDefault
        ? (resolvedThemeMode === ThemeMode.DARK ? DEFAULT_DARK_COLOR : DEFAULT_COLOR)
        : themeColorValue)));
    const [cachedThemeCssVars, setCachedThemeCssVars] = useLocalStorage(THEME_CSS_VARS_KEY, 
    ...
    const setThemeColor = useCallback(async (color) => {
            ...
            unlisten = appWindow.onThemeChanged((e) => {
                if (themeMode.value === ThemeMode.SYSTEM) {
                    ...
                    if (!themeColorValue || themeColorValue === OLD_DEFAULT_COLOR) {
                        const newDefaultColor = newMode === ThemeMode.DARK ? DEFAULT_DARK_COLOR : DEFAULT_COLOR;
    ...
    const setThemeMode = useCallback(async (mode) => {
        ...
        const newResolvedMode = mode === ThemeMode.SYSTEM
            ? window.matchMedia('(prefers-color-scheme: dark)').matches
                ? ThemeMode.DARK
                : ThemeMode.LIGHT
            : mode;
        if (!themeColorValue || themeColorValue === OLD_DEFAULT_COLOR) {
            const newDefaultColor = newResolvedMode === ThemeMode.DARK ? DEFAULT_DARK_COLOR : DEFAULT_COLOR;
            const materialColor = themeFromSourceColor(argbFromHex(newDefaultColor));
            if (!isEqual(materialColor, cachedThemePalette)) {
                setCachedThemePalette(materialColor);
                const themeCssVars = generateThemeCssVars(materialColor);

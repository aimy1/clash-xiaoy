...
export const UpdateProxiesProviders = () => {
    ...
    const handleProviderUpdate = useLockFn(async () => {
        ...
        try {
            setLoading(true);
            await Promise.all(Object.entries(proxiesProvider.data).map(([_, provider]) => provider.mutate()));
        }
        catch (e) {
    ...
    return (<Button variant="contained" loading={loading} startIcon={<Refresh />} onClick={handleProviderUpdate}>
